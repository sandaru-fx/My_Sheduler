...
import { UserProfile, ThemeVariant } from '../models';
import { saveUserProfile } from '../services/api';
import { Button } from '../components/ui/Button';
import { Input } from '../components/ui/Input';
import { User, Palette, Check, Snowflake, Sun, Moon, Sparkles, Flower2, Mail, Briefcase, Info, Calendar, Camera, Shield, Bell, AppWindow } from 'lucide-react';
...
const Profile: React.FC<SettingsProps> = ({ profile, onUpdateProfile }) => {
  ...
  return (
    <div className="h-full flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-6xl mx-auto w-full">
      ...
      {/* Profile Header Card */}
      <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex flex-col md:flex-row items-center gap-10">
          <div className="relative">
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-[2.5rem] bg-gradient-to-br from-indigo-500 to-pink-500 p-1 shadow-2xl transition-transform duration-500 group-hover:rotate-6">
              <div className="w-full h-full rounded-[2.3rem] overflow-hidden border-4 border-slate-900 bg-slate-800">
                <img
            ...
            <button className="absolute -bottom-2 -right-2 p-3 bg-indigo-600 text-white rounded-2xl shadow-xl hover:scale-110 active:scale-95 transition-all border-4 border-slate-900">
              <Camera size={20} />
            </button>
          </div>
          ...
          <div className="flex-1 text-center md:text-left space-y-4">
            <div>
              <h1 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter mb-2">{formData.name}</h1>
              <p className="text-indigo-400 font-black uppercase tracking-[0.3em] text-[10px]">{formData.role}</p>
            </div>
